               ...
               loff_t * offset)
{
	if(list_empty(&dev_message_list)){
		...
		int messageLen = strlen(out_message) + 1;
		mutex_lock(&devLock);
		int copy = copy_to_user(buffer,out_message,messageLen);
...
device_write(struct file *filp,const char *buff, size_t len, loff_t * offset)
{
	...
	if(len>6144){
		...
		char *inp_message = kmalloc(strLen, GFP_KERNEL);
		if(!inp_message)
			return -ENOMEM;
		mutex_lock(&devLock);
		strlcpy(inp_message,buff,strLen);
		message_list *new  = (message_list*)kmalloc(sizeof(message_list),GFP_KERNEL);
		new->message=inp_message;
		INIT_LIST_HEAD(&new->list);
		list_add(&new->list, &dev_message_list);
		mutex_unlock(&devLock);
